	...
	return (
		<tbody className="text-xs text-[#000000] bg-white font-medium">
			{dataArray?.map((project, index) => {
				const { id, name, project_manager, project_vendors, created_at, slug } =
					project;
				const awardee =
					!project_vendors.length === 0 ? "hello" : project_vendors[0];
...
export function AwardeeInfo(props) {
	...
	// role: "",
	const { values, errors, touched, handleChange, index, setFieldValue } =
		props.data;
	const projectError =
		(errors.project_vendors?.length && errors.project_vendors[index]) || {};
	const projectTouched =
		(touched.project_vendors?.length && touched.project_vendors[index]) || {};
	...
	function CheckState() {
		...
		let stat = Object.values(states)?.find(
			(state) => state.name === values.project_vendors[index].state
		);
		return !stat
			? ""
	...
	function CheckZipCode() {
		...
		return zipcode?.cities[values?.project_vendors[index]?.city]?.map(
			(zipcode, index) => {
				return (
					...
					<div className="w-full overflow-auto">
						<div>
							<DashboardSelect {...awardee}>
								<option value={awardee.value && awardee.value}>
									{!awardee.value ? "Select Awardee" : awardee.value}
								</option>
						...
						<div>
							...
							<FormInputContainer
								name={`Search ${values.project_vendors[index].role}`}>
								<input
									list="consultant"
									...
									placeholder={`Search ${values.project_vendors[index].role}`}
									className={`bg-white border border-gray-400 text-gray-500 text-sm rounded focus:outline-[#3B6979] focus:border-[#3B6979] block w-full p-2`}
									disabled={!values.project_vendors[index].role ? true : false}
								/>
								{}
								<datalist id="consultant">
									{[
										...filtered(),
										{ id: "new-vendor", company_name: "Add New Vendor" },
									]?.map((cur, id) => {
										return (
											<option key={cur.id} value={cur.company_name}>
												{cur.consultant_name}
											</option>
										);
									})}
						...
						<FormInputContainer name="Select State">
							<input
								list="states"
								id={`project_vendors.${index}.state`}
								name={`project_vendors.${index}.state`}
								value={values.project_vendors[index].state}
								onChange={handleChange}
								disabled={
							...
							{projectError.state && projectTouched.state && (
						...
						<FormInputContainer name="Select City">
							<input
								list="city"
								id={`project_vendors.${index}.city`}
								name={`project_vendors.${index}.city`}
								value={values.project_vendors[index].city}
								onChange={handleChange}
								className={`bg-white border border-gray-400 text-gray-500 text-sm rounded focus:outline-[#3B6979] focus:border-[#3B6979] block w-full p-2`}
							...
							{projectError.city && projectTouched.city && (
								<Error message={"Field is required"} />
							)}
						</FormInputContainer>
						...
						<FormInputContainer name="Select Zip code">
							<input
								list="zip_code"
								id={`project_vendors.${index}.zip_code`}
								name={`project_vendors.${index}.zip_code`}
								value={values.project_vendors[index].zip_code}
								onChange={handleChange}
								className={`bg-white border border-gray-400 text-gray-500 text-sm rounded focus:outline-[#3B6979] focus:border-[#3B6979] block w-full p-2 `}
							...
							{projectError.zip_code && projectTouched.zip_code && (
								<Error message={"Field is required"} />
							)}
						</FormInputContainer>
						<div>
...
export function CompanyRep({ data }) {
	...
	const company_representative_title = {
		name: "Title",
		id: `project_vendors.${index}.title`,
