				...
				<div className='mx-6 mb-12'>
					<div className='mt-3 ml-2 flex flex-col gap-3 '>
							...
							render={({ remove, push }) => (
								<>
									<div className="flex flex-col gap-3 mt-5">
										<FormInputContainer name="Enter any services the owner provides.">
											<FormInputPlain type={"text"} onKeyPress={(e) => { handleKeyPress(e, push) }} name='service' placeholder="Services" />
										</FormInputContainer>
										<div className="flex gap-2 justify-start">
											{props?.values?.services.map(({ service }, index) => (
												<Fragment key={index}>
													{ 
														service === "" ? <div></div> :
														<div className="flex rounded-md px-2 py-1 w-fit justify-center items-center bg-[#89A5AF] cursor-pointer" onClick={() => remove(index)}>{service}</div>
													}
												</Fragment>
											))}
										</div>
						...
						<FormInputContainer name='What day will the board approve the document?'>
							...
							{props.errors.approveDate && props.touched.approveDate && <Error message={props.errors.approveDate} />}
						</FormInputContainer>
