...
export function TableBody({ skip, perPage, dataArray, onDownload }) {
  return (
    <table className=" border-separate [border-spacing: 0 15px] w-full">
    ...
    // <section className="w-full text-base font-medium overflow-hidden container mx-auto px-4 lg:px-12">
    //   {dataArray.slice(skip, skip + perPage).map((manager, index) => {
    //     const { id, title, category, downloadLink } = manager;
    //     return (
    //       <ul
    //         key={id}
    //         className="border-b border-[#D8E1E4] bg-white rounded-lg h-16 flex justify-between items-center mb-4 width-[100%] flex-nowrap"
    //       >
    //         <li
    //           // scope="row"
    //           className="py-4 px-4 font-normal capitalize text-gray-900 whitespace-nowrap w-[50%]"
