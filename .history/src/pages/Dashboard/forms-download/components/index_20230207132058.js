...
export function TableBody({ onDownload, skip, perPage, dataArray }) {
  return (
    <table
      className=" border-separate [border-spacing: 0 15px]"
    ...
    >
      <tbody className="w-full text-base font-medium">
        {dataArray.slice(skip, skip + perPage).map((manager, index) => {
          const { id, title, category } = manager;
          return (
            <tr
              key={id}
            ...
            >
              <td
                // scope="row"
                className="py-4 px-4 font-normal capitalize text-gray-900 whitespace-nowrap w-[40%]"
              >
                {title}
              </td>
              <td
                // scope="row"
                className="py-4 px-4 font-normal capitalize text-[#693B79] whitespace-nowrap"
              >
                {category}
              </td>
              <td className="py-4 px-4 flex items-center justify-start gap-3">
                <a
                  href="/https://durhamapp.000webhostapp.com/pdfs/Oderinde%20Emmanuel%20Resume_3.pdf"
