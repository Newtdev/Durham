...
export function TableBody({ skip, perPage, dataArray, onDownload }) {
  return (
    ...
    //   </tbody>
    // </table>
    <main className="w-full text-base font-medium overflow-hidden">
      {dataArray.slice(skip, skip + perPage).map((manager, index) => {
        const { id, title, category, downloadLink } = manager;
        return (
          <ul
            key={id}
            className="border-b border-[#D8E1E4] bg-white rounded-lg h-16 flex justify-between items-center mb-4"
          >
            <li
              // scope="row"
