                ...
                <>
                  <div>
                    {props?.values?.items?.map((item, index) => (
                      <Fragment key={index}>
                        <FormInputContainer name={`Item ${index + 1}`}>
                          <FormInputPlain
                            type={"text"}
                            onChange={props.handleChange}
                            name="What items should the Vendor include in their RFP responses? Enter the items in the order that the vendor has to set up."
